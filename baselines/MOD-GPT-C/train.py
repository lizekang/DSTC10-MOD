...
def train(model, tokenizer, optimizer, dataset, epoch): 
    ...
    for instance in dataset: 
        ...
        if iteration % print_freq == 0:
            ...
            'Retrieval Acc {mac:.3f}'.format(epoch, iteration, len(dataset),loss=avg_loss, acc=avg_acc, mac=float(meme_correct_num/meme_total_num))) 
        iteration += 1 
...
def meme_retrieval_compute(cur_img_feature, target_img_feature, cat_img_features): 
    ...
    cur_dist = torch.sqrt(torch.sum((cur_img_feature - target_img_feature)**2, dim=1))
