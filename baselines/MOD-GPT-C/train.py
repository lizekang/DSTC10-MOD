...
gpt_path = 'ckpt/CDial-GPT_LCCC-base'
ckpt_usage = False
lr = 6e-5
epochs = 8
...
def main():
    ...
    # ckpt_path = "./ckpt/mod_gpt/new_epoch_0_loss_10.578"
    ...
    for instance in dataset:
        ...
        if iteration % print_freq == 0:
            print('Epoch:[{0}][{1}/{2}]\t'
                  ...
                  'Retrieval Acc {mac:.3f}'.format(epoch,
                                                   iteration,
                                                   len(dataset),
                                                   loss=avg_loss,
                                                   acc=avg_acc,
                                                   mac=float(meme_correct_num /
                                                             meme_total_num)) if meme_total_num > 0 else 0)
        iteration += 1
...
def meme_retrieval_compute(cur_img_feature, target_img_feature,
    ...
    cur_dist = torch.sqrt(
        torch.sum((cur_img_feature - target_img_feature)**2, dim=1))
