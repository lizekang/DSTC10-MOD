...
def main(): 
    ...
    parser.add_argument("--fp16", type=int, default=0, help='O0, O1, O2, or O3') 
    ...
    # model initialize 
    if ckpt_usage == True: 
        tokenizer = BertTokenizer.from_pretrained('ckpt/mod_gpt', do_lower_case=True) 
...
def train(args, model, tokenizer, optimizer, dataset, epoch): 
    ...
    for instance in dataset: 
        ...
        loss = model(input_embs, token_type_ids, labels, img_feature, meme_flag) 
        ...
        if iteration % print_freq == 0:
            print('Epoch:[{0}][{1}/{2}]\t'
            'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(epoch, iteration, len(dataset),loss=avg_loss))
    ...
    with torch.no_grad(): 
        for instance in dataset: 
            ...
            if input_embs.size(-2) > 450:
                continue
            img_feature = history_img[-1, :].unsqueeze(0) 
            loss, mf_logits, lm_logits, cur_img_feature = model(input_embs, token_type_ids, labels, img_feature, meme_flag, 'val') 
            ...
            if iteration % print_freq == 0:
                ...
                'Meme Retrieval {mac:.3f}'.format(epoch, iteration, len(dataset),loss=avg_loss, acc=avg_acc, mac=float(meme_correct_num/meme_total_num))) 
            iteration += 1 
            #break 
